 ...
 */
package com.ytrain.mutrain.indicator;
...
/**
 * 主页面实现轮播效果的viewpager指示器 Draws circles (one for each view). The current view
 * position is filled and others are only stroked.
 */
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int iLoop = 0; iLoop < count; iLoop++) {
			...
			if (mPaintPageFill.getAlpha() > 0) {
				...
				RectF oval1 = new RectF(dX - pageFillRadius, dY
						- pageFillRadius, dX + pageFillRadius, dY
						+ pageFillRadius);// 设置个新的长方形
				canvas.drawRoundRect(oval1, 1, 1, mPaintPageFill);// 第二个参数是x半径，第三个参数是y半径
			...
			if (pageFillRadius != mRadius) {
				...
				RectF oval2 = new RectF(dX - mRadius, dY - mRadius, dX
						+ mRadius, dY + mRadius);// 设置个新的长方形
				canvas.drawRoundRect(oval2, 1, 1, mPaintStroke);// 第二个参数是x半径，第三个参数是y半径
				// 画边缘线
		...
		// 画圆形的填充图
		// canvas.drawCircle(dX, dY, mRadius+3, mPaintFill);
		// 画长方形的填充图
		RectF oval3 = new RectF(dX - mRadius - 1, dY - mRadius, dX + mRadius
				+ 1, dY + mRadius);// 设置个新的长方形
		canvas.drawRoundRect(oval3, 2, 2, mPaintFill);// 第二个参数是x半径，第三个参数是y半径
	}
